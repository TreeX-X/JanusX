...
const captions = {
  'terminal-split': ['01 / CORE', '拖拽标签分屏，让两个终端并肩工作'],
  'right-sidebar': ['02 / EVERYDAY', '切换右侧工具，在终端旁查看项目文件'],
...
for (const recording of recordings) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    previous = opaque
    if (i === files.length - 1) await sharp(raw, {raw:{width:1280,height:864,channels:4}}).png().toFile(`wiki/assets/${recording.name}.png`)
  }
  gif.finish()
  await writeFile(`wiki/assets/${recording.name}.gif`, gif.bytes())
