...
| Language service | `src/main/language-service/clangd-client.ts`, `src/main/language-service/clangd-manager.ts`, `src/main/ipc/language-service-handlers.ts` | clangd LSP client/manager for go-to-definition |
...
| Agent runtime | `src/main/agent/runtime/*`, `src/main/agent/runtime/tools/*`, `src/main/ipc/agent-runtime-handlers.ts` | workspace-scoped tool execution with policy gate, path guard, file transactions |
| Janus agent loop | `src/main/agent/loop/janus-agent-loop.ts`, `src/main/agent/loop/vercel-stream-adapter.ts`, `src/main/agent/loop/runtime-tool-adapter.ts` | structured agent loop with Vercel AI SDK streaming, before/after tool hooks |
| Agent environment | `src/main/agent/environment/janus-workspace-fs.ts` | workspace filesystem access, evidence context, text buffer detection |
...
| Chat orchestration | `src/main/llm/chat-orchestrator.ts`, `src/main/llm/ai-runtime.ts`, `src/main/llm/workspace-chat-tools.ts` | chat message/stream management, tool trace, workspace mutation intent, AI SDK runtime |
...
| Knowledge | `src/main/knowledge/*`, `src/main/ipc/knowledge-handlers.ts` | observations, search (BM25/tokenizer), context, recall, review, truth, extraction, retention, operations, MCP, agent turn recorder |
| Office | `src/main/office/*`, `src/main/ipc/office-handlers.ts` | guarded Office CLI installation, artifacts, watchers, previews, exports, broker, skills, project rules |
...
| Janus island/chat | `components/janus/*` | titlebar island, eye, identity core, expanded UI, chat pane/provider, conversations, tool call card, project candidate, resources, runtime state, streaming printer, island gesture, knowledge peek |
