...
test('built desktop implements, checks, persists receipts and reloads local history', async () => {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      requests.push(body.stream ? 'implementation' : 'review')
      if (body.stream) {
        const current = turn++
        const tool = current === 0 ? 'workspace_read' : 'workspace_edit'
        if (current < 2) expect(body.tools.some((entry: { name: string }) => entry.name === tool)).toBe(true)
        if (current === 1) expect(JSON.stringify(body.input)).toContain('42')
        const args = current === 0 ? { path: 'src/value.txt' } : {
        ...
        if (current < 2) {
          ...
          emit({ type: 'response.output_text.delta', item_id: item.id, delta: 'Updated src/value.txt to 43.' })
          emit({ type: 'response.output_item.done', output_index: 0, item })
        }
        ...
        expect(prompt.includes('[command/passed]'), 'Review must receive a passed file check').toBe(true)
        const content = JSON.stringify({ verdict: 'approved', coverage: [{ uri: match[1], criterionId: match[2], criterionHash: match[3], checkIds: ['V-1'] }], summary: 'The declared file check passes.' })
        response.writeHead(200, { 'Content-Type': 'application/json' })
        response.end(JSON.stringify({ id: 'review', created_at: 1, model: 'fixture-model', output: [{ type: 'message', id: 'review-text', role: 'assistant', content: [{ type: 'output_text', text: content, annotations: [] }] }], usage: { input_tokens: 10, output_tokens: 10 } }))
      }
    } catch (error) {
  ...
  try {
    ...
    const prepared = await page.evaluate(async ({ cwd, taskUri }) => {
      const api = (window as HarnessWindow).electron.harness
      const prepared = await api.runPrepare(cwd, { taskUri, mode: 'xdo', closeout: 'commit-required' })
      await api.runStart(cwd, prepared.runId, 'desktop', { by: 'desktop' })
      return prepared
    }, { cwd: workspace, taskUri })
    ...
    expect(requests).toEqual(['implementation', 'implementation', 'implementation', 'review'])
