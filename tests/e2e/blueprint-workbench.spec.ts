...
for (const viewport of [{ width: 1440, height: 900 }, { width: 1280, height: 720 }]) {
  test.describe(`${viewport.width}x${viewport.height} current-source Workbench`, () => {
    ...
    test('two compact bars and three ordered panels keep the composer in the viewport', async ({ page }) => {
      ...
      await expect(chat(page).locator('.bp-maintenance-settings')).not.toHaveAttribute('open', '')
      await expect(chat(page).locator('.bp-maintenance-audits')).not.toHaveAttribute('open', '')
    ...
    test('selection, detail restore and real controller send survive panel collapse', async ({ page }) => {
      ...
      expect(request).toMatchObject({ domain: 'project', noteRefs: [{ uri: `note://${repoId}/${childId}`, expectedHash: 'a'.repeat(64), checkoutPath: 'C:/fixture' }], workspaceResources: [{ workspaceId: 'ws', workspacePath: 'C:/fixture' }] })
    ...
    test('expanded task settings and pending approval leave a usable chat region', async ({ page }) => {
      await open(page)
      const settings = chat(page).locator('.bp-maintenance-settings')
      await settings.locator(':scope > summary').click()
      await settings.locator('textarea').fill('Inspect the selected root Note')
      await insideViewport(page, chat(page).locator('.janus-chat textarea'))
      await settings.getByRole('button', { name: '整理为提案', exact: true }).click()
      const proposal = chat(page).locator('.bp-maintenance-proposal')
      await expect(proposal).toBeVisible()
      await expect(proposal.locator('.bp-maintenance-approval-operation')).toHaveCount(4)
      await expect(chat(page).locator('.bp-maintenance-task-details')).not.toHaveAttribute('open', '')
      await expect(chat(page).locator('.bp-maintenance-audits')).not.toHaveAttribute('open', '')
      await expect(chat(page)).toContainText('Project reply in progress')
      const controls = chat(page).locator('.bp-maintenance-controls')
      const controlsBounds = await rect(controls)
      const panelBounds = await rect(chat(page))
      expect(controlsBounds.height).toBeLessThan(panelBounds.height / 2)
      ...
      await proposal.locator(':scope > summary').click()
      await expect(proposal).toHaveAttribute('open', '')
      const request = await page.evaluate(() => (window as any).projectFixture.streams.at(-1))
      expect(request.maintenanceTaskId).toBe('maintenance-1')
      expect(await page.evaluate(() => (window as any).projectFixture.maintenance.applies)).toEqual([])
    })
  })
}
