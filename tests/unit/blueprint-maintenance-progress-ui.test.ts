...
describe('Blueprint maintenance progress UI', () => {
  it('keeps progress integrated and provides a quick return to the conversation bottom', async () => {
    ...
    expect(panel).toContain("import { ThinkingRegion } from '../janus/ThinkingRegion'")
    expect(panel).toContain("import { ToolCallGroup } from '../janus/ToolCallCard'")
    expect(panel).toContain('<ThinkingRegion snapshot={taskReasoning} streaming />')
    expect(panel).toContain('<ToolCallGroup entries={taskToolTraces.slice(-5).map(toChatTraceEntry)} workspaceNames={workspaceNameMap} />')
