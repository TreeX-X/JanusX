...
describe('product workspace kind mapping', () => {
  it('routes office, markdown and html files while rejecting the rest', () => {
    ...
    expect(productKindForPath('diagram.png')).toBe('image')
    expect(productKindForPath('chart.SVG')).toBe('image')
    expect(productKindForPath('notes.txt')).toBe('text')
    expect(productKindForPath('config.json')).toBe('text')
    expect(productKindForPath('data.csv')).toBe('text')
    expect(productKindForPath('archive.zip')).toBe('unsupported')
...
describe('product workspace lifecycle', () => {
  ...
  it('opens local kinds without a preview lease and bumps revision on reload', async () => {
    ...
    await store.getState().openPreview('workspace', 'diagram.png')
    expect(store.getState().tabs[0]).toMatchObject({ status: 'ready', kind: 'image' })
    await store.getState().openPreview('workspace', 'config.json')
    expect(store.getState().tabs[1]).toMatchObject({ status: 'ready', kind: 'text' })
    expect(service.startPreview).not.toHaveBeenCalled()
    expect(service.stopPreview).not.toHaveBeenCalled()
  })
