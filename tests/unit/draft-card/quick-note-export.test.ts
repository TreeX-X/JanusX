...
import { buildHtmlDocument, exportDraftCard, mdToPayload, stripMarkdown } from '../../../src/renderer/src/components/note/quick-note-export'
import type { DraftCard } from '../../../src/renderer/src/stores/draft-card'

const draft: DraftCard = { id: '12345678-x', terminalId: 't', title: 'Release/Note', content: '# Title\n\n- item\n\n```ts\nconst x = 1\n```', createdAt: 1, updatedAt: 1 }
...
describe('quick note export', () => {
  ...
  it('preserves markdown and strips common markers for text', () => {
    expect(mdToPayload(draft.content, 'md')).toBe(draft.content)
    expect(stripMarkdown(draft.content)).toBe('Title\n\nitem\n\nconst x = 1')
  })
  ...
  it('uses dialog then positional file save and treats cancellation silently', async () => {
    ...
    await expect(exportDraftCard(draft, 'md')).resolves.toBe('saved')
    expect(saveFile).toHaveBeenCalledWith({ defaultName: 'Release-Note.md', extension: 'md' })
    expect(save).toHaveBeenCalledWith('C:/Release-Note.md', draft.content)
    saveFile.mockReset().mockResolvedValueOnce({ canceled: true })
    save.mockReset()
    await expect(exportDraftCard(draft, 'txt')).resolves.toBe('canceled')
    expect(saveFile).toHaveBeenCalledTimes(1)
    expect(save).not.toHaveBeenCalled()
    saveFile.mockReset().mockResolvedValueOnce({ canceled: false })
    await expect(exportDraftCard(draft, 'html')).resolves.toBe('canceled')
    expect(saveFile).toHaveBeenCalledTimes(1)
    expect(save).not.toHaveBeenCalled()
  })
  it('surfaces save errors for the view to display', async () => {
    ...
    await expect(exportDraftCard(draft, 'txt')).rejects.toThrow('disk full')

    saveFile.mockReset().mockRejectedValueOnce(new Error('dialog unavailable'))
    await expect(exportDraftCard(draft, 'html')).rejects.toThrow('dialog unavailable')
  })
})
