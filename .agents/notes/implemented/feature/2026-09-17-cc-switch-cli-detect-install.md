...
- **Gains**: Settings shows Claude Code state (not installed, broken, ready, update available) with current and latest versions, and installs or upgrades in one click; 13 unit tests pin the probe priority, the broken-state distinction, the `cmd call` invocation shape, the busy-guard race, and the semver boundary (`tests/unit/cc-switch/`).
- **Costs and limits**: Install needs npm on a discoverable path, otherwise the card degrades to a copyable manual command by design; latest lookup needs registry access and degrades to unknown offline. The busy guard is process-local, so two JanusX instances could still race npm. Adding the next tool means a new registry row plus its detector and projector, not a framework change; provider credential switching stays out of scope until the profile-store phase lands.
