...
Settings opens on general: `AppSettingsModal` defaults `initialTab` to `general` and the Titlebar logo entry opens `general`, while team deep-links and the updater badge keep their explicit targets. Provider storage stays JSON in `llm-config.json` with a `terminalBindings` map for `janus`, `claude`, `codex`, `opencode`, and `pi`; each entry holds a nullable `providerId` plus an optional model override, null means follow the global default, unknown providers are rejected on write, and deleting a provider returns its bindings to follow-default. Janus chat resolves the Janus binding first through `getJanusEffectiveProvider` and falls back to the global default, so an unset Janus binding preserves existing behavior. Anthropic ships as a first-class `anthropic` auth type with an `AnthropicAdapter` on the Messages API (`x-api-key` plus `anthropic-version 2023-06-01`) backed by `@ai-sdk/anthropic@1`, and Claude sync accepts `api-key` and `anthropic` credentials alike. The engine tab renders providers, then one matrix section with a provider select plus model override per terminal and Claude sync plus rollback inline, then the add-provider form with OpenAI, Anthropic, and Vertex options.
...
- **Costs and limits**: Only Claude performs a live file write with backup plus verify; Codex, OpenCode, Pi, and Janus persist bindings only, and their live projectors wait for per-dialect appliers with acceptance passes. Sync state stays Claude-scoped in `cc-switch-sync.json`. The Anthropic adapter adds `@ai-sdk/anthropic@1` to the llm-core closure. Revisit when a second CLI needs a live write or when the matrix outgrows one record per tool.
