...
- **Costs and limits**: Codex, OpenCode, and Pi project exactly one model key each through per-format projectors, and Janus stays internal-only; details live in [the tabbed follow-up](./2026-09-18-terminal-tabs-per-format-projectors.md). Sync state stays Claude-scoped in `external-cli-sync.json` (legacy file adopted once). The Anthropic adapter adds `@ai-sdk/anthropic@1` to the llm-core closure. Revisit when a second CLI needs credential sync or when the tab row outgrows a handful of terminals.
