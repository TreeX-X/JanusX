...
`TerminalStatus` in `src/shared/ipc/terminal.ts` keeps six values for hook routing and sort order. The renderer projects approval and input onto one attention visual in `src/renderer/src/lib/terminal-sidebar-visual.ts`: both map to the `terminal:status.needs-action` label with the orange treatment, and `Sidebar.tsx` renders both as one pulsing attention ring (ring form owned by [terminal status ring](./2026-09-19-terminal-status-ring.md)). `TERMINAL_ATTENTION_ORDER` still places approval before input, and `summarizeTerminalActivity` still counts each state separately, so list order and the workspace badge keep their priority while the row indicator carries five visuals instead of six.
