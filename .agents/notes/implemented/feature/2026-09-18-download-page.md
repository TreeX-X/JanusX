...
The page prioritizes package selection, followed by installation steps, core functionality and explicitly experimental capabilities. Six GIF recordings show terminal splitting, right-sidebar tools, the standalone Janus CLI, roundtable preparation, blueprint browsing and knowledge review. The split recording uses the desktop Janus and Claude presets with Janus-agentX on the left and Claude Code on the right. The sidebar recording keeps Janus-agentX beside the file and Git tools and embedded editor. The copy states that these CLIs require installation and their own login or model configuration for model tasks.

The recordings use an isolated profile and an actual source checkout; manually authored blueprint and knowledge content is identified in the copy. No model-generated result is represented by the preparation recordings. Temporary Claude configuration, credential isolation and terminal color settings belong to the application repository's [product recording Note](https://github.com/TreeX-X/JanusX/blob/main/.agents/notes/implemented/feature/2026-09-18-product-tour.md).
