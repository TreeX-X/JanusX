...
The host supports one checkout and internal xdo. It stores bounded implementation observations, attempt summaries and policy audit events. Interactive clarification remains absent. Tool failures refuse verification; users can correct the blocker and retry. Real provider acceptance, delegated execution, cross-machine orchestration, full protocol recovery and versioned releases remain separate work in [S9 readiness](2026-09-18-harness-s9-readiness.md).
