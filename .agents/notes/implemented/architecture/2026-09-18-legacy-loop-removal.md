...
- **Costs and limits**: legacy JSON discussion stays frozen and starts refuse until migration; unlinked tasks settle pending work without new discussion. Panel reasoning and trace state keep their fields with no producer behind them. Removed-button locale keys stay unused. The island run shows 3 failures in Knowledge-peek and Island-Chat areas with no maintenance reference, failing identically in isolation. Revisit when migration consumes the remaining JSON blueprints; the kept legacy apply lane and JSON reads can retire then.
