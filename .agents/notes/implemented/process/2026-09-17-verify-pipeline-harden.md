...
- **Gains**: 同级构建从两次全量（含一次必败）降为一次定序构建，run 注解不再出现构建假 error；`Unit tests` 等具名步骤让失败阶段一眼可辨；`office-artifact-index` 11 个用例本地全绿；`officecli-installer` 在 CI 模式下本地全绿，并行负载崩溃的诱因消除。
- **Costs and limits**: CI 与本地的 vitest 调度存在分歧，本地复现 CI 行为需显式置 `CI=true`；workflow 的分步命令是 `package.json` 链的人工镜像，改链必须同步两处，遗忘即漂移。门禁仍未全绿：`janus-ipc-contract`、`janus-resource-ui` 等产品侧失败与 `janus-agentX` 仓的排序归属保持原判，跟踪仍在 issue #1，合并继续走管理 bypass 并留书面理由，直到 owning workstream 落地。
