...
|---|---|---|---|
| 1 | 能力注册表 | 砍 | 待确认 |
| 2 | 接口一等字段 | 版本道（有条件） | 待确认 |
| 3 | 机制行为栏 | 砍 | 待确认 |
| 4 | 边界栏 role 三档 | 后议 | 待确认 |
| 5 | 反链 | P2 实现层 | 待确认 |
| 6 | 摘要栏 | 不加字段 | 待确认 |
| 7 | 接口表进索引 | 不加字段 | 待确认 |
| 8 | 多任务调度 | S9 另立项 | 待确认 |
| 9 | AC 编号错配 | 工具层 | 待确认 |
| 10 | hash 雪崩放宽 | 维持 | 待确认 |
| 11 | lifecycle/execution 双轨 | 不并 | 待确认 |
| 12 | 接口载体结构化 | 约定层 | 待确认 |
| — | §9 已砍 8 项（验收/验证/仓库/relation/文件名/class-tags/状态/版本单钉） | 默认不开启 | 需点名才重开 |
