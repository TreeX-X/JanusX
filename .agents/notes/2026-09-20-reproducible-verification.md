...
Desktop fixtures explicitly set `JANUSX_KNOWLEDGE_ROOT`; changing Electron's user-data path or HOME alone does not isolate knowledge storage on Windows. Geometry assertions wait for finite workbench animations, and closed dock assertions check opacity plus inert and aria-hidden because the mounted shell deliberately retains its layout box.
...
The three repaired desktop scenarios pass targeted Playwright runs: blueprint card layout, workspace/terminal/project paths, and knowledge processing. Lint reports zero errors and 60 existing warnings. The OfficeCLI release artifact download matches the pinned SHA256.
