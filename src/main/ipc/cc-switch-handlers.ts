...
export function registerCcSwitchHandlers(options: RegisterCcSwitchHandlersOptions): () => void {
  ...
  const channels = [CC_SWITCH_CHANNELS.detect, CC_SWITCH_CHANNELS.latest, CC_SWITCH_CHANNELS.install] as const

  for (const channel of channels) {
    ipcMain.handle(channel, async (event, toolId: unknown) => {
      ...
      if (!isSupportedToolId(toolId)) {
        if (channel === CC_SWITCH_CHANNELS.latest) return { toolId: fallbackTool }
        if (channel === CC_SWITCH_CHANNELS.install) return { toolId: fallbackTool, success: false, error: 'Unsupported tool.' }
        return { toolId: fallbackTool, installed: false, runnable: false, hint: 'Unsupported tool.' }
      }
      try {
        if (channel === CC_SWITCH_CHANNELS.detect) return service.detect(toolId)
        if (channel === CC_SWITCH_CHANNELS.latest) return service.latest(toolId)
        return service.install(toolId)
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error)
        if (channel === CC_SWITCH_CHANNELS.latest) return { toolId }
        if (channel === CC_SWITCH_CHANNELS.install) return { toolId, success: false, error: message }
        return { toolId, installed: false, runnable: false, hint: message }
      }
    })
  }
